class useJsonConfig {
    __New(configTemplateSrc, configFileName, configDest := A_MyDocuments) {
        ...
        if (curConfig is Error) {
            ...
            this.currentConfig := useJsonConfig.JSON.parse(FileRead(this.path, "UTF-8"))
        } else {
    ...
    updateKeys() {
        localConfig := useJsonConfig.JSON.parse(FileRead(this.path, "UTF-8"))
        tempConfig := useJsonConfig.JSON.parse(FileRead(this.configTemplateSrc, "UTF-8"))
    ...
    _readFirstMatch(key, config) {
        if (config.Has(key)) {
            result := config[key]
            return IsObject(result) ? useJsonConfig.JSON.parse(useJsonConfig.JSON.stringify(result)) : result
        }
    ...
    class JSON {
        ...
        static parse(text, keepbooltype := false, as_map := true) {
            try {
                keepbooltype ? (_true := useJsonConfig.JSON.true, _false := useJsonConfig.JSON.false, _null := useJsonConfig.JSON.null) : (_true := true, _false := false, _null := "")
                        ...
                        if (t := Trim(A_LoopField, " `t`r`n")) = "," || (t = ":" && V := 1)
                            ...
                            Loop Parse t {
                                ...
                                if InStr("`n`r `t", A_LoopField)
                                    ...
                                    if !A && !V
                                        throw Error("Malformed JSON - missing key.", -1, t)
            ...
            ES(S) {
                switch Type(S) {
                    ...
                    case "String":
                        ...
                        return '"' S '"'
                    default:
                        return S == useJsonConfig.JSON.true ? "true" : S == useJsonConfig.JSON.false ? "false" : "null"
                }
            }
            CL(i) {
                Loop (s := "", space ? i - 1 : 0)
